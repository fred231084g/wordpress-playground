...
describe('ensureWpConfig', () => {
	...
	it('should define required constants when they are missing', async () => {
		...
		expect(rewritten).toContain(`define( 'DB_PASSWORD', 'wordpress' );`);
		expect(rewritten).toContain(`define( 'DB_HOST', 'localhost' );`);
		expect(rewritten).toContain(`define( 'DB_CHARSET', 'utf8mb4' );`);
	...
	it('should only define missing constants', async () => {
		...
		expect(rewritten).not.toContain(`define( 'DB_USER', 'wordpress' );`);
...
`);
	...
	it("should remove the injected configuration when it's no longer needed", async () => {
		...
		expect(rewritten1).toContain(`define( 'DB_NAME', 'wordpress' );`);
		...
		expect(rewritten2).not.toContain(`define( 'DB_NAME', 'wordpress' );`);
